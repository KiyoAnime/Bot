...
export const run: ChatCmdRun = async (client, interaction) => {
    ...
    if (!types.includes(channel.type)) {
        return interaction.reply({ content: `Invalid channel type. I cannot speak in that channel. I can only speak in text channels.`, ephemeral: true });
    }
    const newChannel = channel as TextChannel | ThreadChannel;
    if (!newChannel.permissionsFor(interaction.guild?.members.me!).has(PermissionsBitField.Flags.SendMessages)) {
        return interaction.reply({ content: 'I do not have permission to speak in this channel. I can only speak in public channels.', ephemeral: true });
    }
