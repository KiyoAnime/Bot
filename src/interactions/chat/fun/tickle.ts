...
export const run: ChatCmdRun = async (client, interaction) => {
    ...
    const img = await axios.get('https://nekos.life/api/v2/img/tickle');
    const embed = new EmbedBuilder({
        title: `${interaction.user.username} tickels ${member.user.username}!`,
    ...
    options: [
        {
            ...
            name: 'member',
            required: true,
            description: 'The member to tickle.'
        }
    ]
};
